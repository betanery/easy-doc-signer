...
const Landing = () => {
  const benefitsSection = useScrollReveal({ threshold: 0.1 });
  const plansSection = useScrollReveal({ threshold: 0.05 });
  const faqSection = useScrollReveal({ threshold: 0.1 });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section 
        ...
        id="beneficios" 
        className={`py-16 md:py-20 bg-gradient-to-b from-background to-muted/30 transition-all duration-1000 ${
        ...
        <div className="container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {benefits.map((benefit, index) => (
              <Card 
                key={index} 
                className={`group hover:shadow-premium transition-all duration-500 hover:-translate-y-2 border-primary/20 hover:border-primary/40 overflow-hidden relative ${
                  benefitsSection.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                }`}
                style={{ 
      ...
      <section 
        ...
        className={`py-16 md:py-20 transition-all duration-1000 ${
          plansSection.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
        }`}
      >
        <div className="container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {plans.map((plan, index) => (
              <Card 
                key={index} 
                className={`relative hover:shadow-xl transition-all duration-700 ${
                  plan.popular ? 'border-primary shadow-lg scale-105' : ''
                } ${
                  plansSection.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                }`}
                style={{ 
                  transitionDelay: plansSection.isVisible ? `${index * 150}ms` : '0ms'
                }}
              >
                {plan.popular && (
      ...
      <section 
        ref={faqSection.ref}
        id="faq" 
        className={`py-16 md:py-20 bg-secondary transition-all duration-1000 ${
          ...
          <Accordion type="single" collapsible className="space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem 
                key={index} 
                value={`item-${index}`} 
                className={`bg-background rounded-lg px-6 border transition-all duration-500 ${
                  faqSection.isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'
                }`}
                style={{ 
